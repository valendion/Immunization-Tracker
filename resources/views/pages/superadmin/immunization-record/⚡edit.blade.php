...
new class extends Component {
    ...
    public ImmunizationRecord $immunizationRecord;

    public function mount()
    {
        // Ambil data berdasarkan ID di URL
        $this->immunizationRecord = ImmunizationRecord::findOrFail($this->id);
        ...
        try {
            $this->validate(
                [
                    ...
                    'vaccine_id' => ['required', Rule::unique('immunization_records')->ignore($this->immunizationRecord->id)->where('child_id', $this->child_id)],
            ...
            $this->immunizationRecord->update([
            ...
            session()->flash('swal', [
                'icon' => 'success',
                'title' => 'Success!',
                'message' => 'Immunization record Successfuly Updated',
                'timer' => 2000,
            ]);

            $this->redirect('/superadmin/immunization-record');
...
@script
    <script>
        function initSelect2() {
            ...
            // SYNC nilai awal dari Livewire → Select2
            $('#select_child').val(@this.child_id).trigger('change');
            $('#select_vaccine').val(@this.vaccine_id).trigger('change');
            $('#select_facility').val(@this.health_facility_id).trigger('change');

            // KUNCI: sinkron Select2 → Livewire
            $('.select2bs4').off('select2:select select2:unselect')
